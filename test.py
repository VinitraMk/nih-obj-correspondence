...
def main():
    ...
    data_fol_name=sys.argv[2]#'test_5'
    output_file_name=sys.argv[3]#'out.csv'
    cwd = os.path.join(cwd,data_fol_name)
    #print('\nCurrent working directory:',cwd)
    N_CLASSES=15
    model=AlexNet(N_CLASSES)
    model_file_name='C:\\Users\\mushu\\Desktop\\PythonTest\\AlexNet_Epoch_4.pkl'
    ...
    for i in range(len(labels)):
        ...
        if(fname==data_files[i]):
            ...
            if torch.no_grad():
                ...
                probs=sigm(output)[0]
                #print(probs)
                indices=np.argwhere(probs>=0.066)[0]
                pred_label_list=[]
